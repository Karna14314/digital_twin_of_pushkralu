...
function ServicesView() {
  const serviceRows = [
    { name: 'Medical capacity', available: 40, total: 100, detail: 'Rehearsal state · official plan: 30 field hospitals and 523 medical support vehicles' },
    { name: 'Toilets', available: 92, total: 100, detail: 'Rehearsal readiness · authoritative inventory pending' },
    { name: 'Drinking water', available: 91, total: 100, detail: 'Rehearsal tank state · utility meters pending' },
    { name: 'Food counters', available: 89, total: 100, detail: 'Rehearsal readiness · official inventory pending' },
    { name: 'Lost & found', available: 80, total: 100, detail: 'Rehearsal desk capacity · 37 modelled open reports' }
  ]
  return <div className="services-layout"><section className="twin-card services-card"><div className="card-heading"><div><div className="eyebrow">Rehearsal capacity board</div><h2>Essential services</h2></div><span className="tiny-label">Simulation state</span></div>{serviceRows.map(item => { const value = Math.min(100, item.available / item.total * 100); return <div className="service-row" key={item.name}><div><b>{item.name}</b><span>{item.detail}</span></div><div className="service-meter"><i><em style={{ width: `${value}%` }} /></i><small>{value}% simulated ready</small></div><span className={value < 55 ? 'warning-text' : 'positive'}>{value < 55 ? 'Watch' : 'Ready'}</span></div>})}</section><section className="twin-card water-card"><div className="eyebrow">River operations</div><h2>Godavari safety envelope</h2><div className="water-gauge"><div><Waves size={25} /><span><b>42.10 ft</b>Dowleswaram modelled level</span></div><i><em style={{ width: '78%' }} /><b className="warning-line" /></i><div className="gauge-labels"><span>Normal 36</span><span>Warning 43</span><span>Severe 48</span></div></div><div className="water-facts"><span><b>+0.18 m</b>90 min rise</span><span><b>1.4 m/s</b>mid-channel speed</span><span><b>0.7 m</b>forecast freeboard</span></div><div className="field-confirm"><CheckCircle2 size={16} /><span><b>No closure rule active</b>Field gauge confirmation still required before operational use.</span></div></section></div>
}

export function ResourcesPage() {
  return <div className="page-content"><div className="page-intro"><div><div className="eyebrow">Resource ledger</div><h1>Fleet & response assets</h1><p>Official 2027 plan totals with separately labelled rehearsal availability.</p></div><button className="primary-button"><Navigation size={16} /> Dispatch board</button></div><div className="resource-page-grid">{resources.map(resource => { const percentage = Math.max(3, resource.available / resource.total * 100); const Icon = resource.type === 'boat' ? Ship : resource.type === 'ambulance' ? Ambulance : resource.type === 'bus' ? Bus : resource.type === 'medical' ? Activity : Users; return <section className="twin-card resource-detail-card" key={resource.id}><div className={`large-resource-icon ${resource.type}`}><Icon size={25} /></div><div className="card-heading"><div><div className="eyebrow">{resource.id} · {resource.domain}</div><h2>{resource.name}</h2></div><span className={`status-pill ${resource.status}`}>{resource.basis === 'official-plan' ? '2027 plan' : 'rehearsal'}</span></div><div className="asset-total"><b>{resource.total.toLocaleString('en-IN')}</b><span>official planned total</span></div><div className="asset-bar"><i style={{ width: `${percentage}%` }} /></div><div className="asset-facts"><span><b>{resource.available}</b>Rehearsal available</span><span><b>{resource.committed}</b>Sim committed</span><span><b>{resource.updatedAt}</b>Simulation update</span></div><button className="secondary-button full">Open unit ledger <ArrowRight size={15} /></button></section> })}</div></div>
}
